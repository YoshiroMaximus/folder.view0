...
const createFolders = async () => {
    ...
    // The order Unraid will actually start containers in, taken from the autostart file. State.Autostart
    // is only a boolean, so sorting on it (as this used to) is a no-op that leaves containersInfo in
    // Docker's creation order -- which never matches the UI, making the warning below always fire.
    const autostartOrder = Object.values(containersInfo)
        .filter(el => Number.isInteger(el.info.State.AutostartIndex))
        .sort((a, b) => a.info.State.AutostartIndex - b.info.State.AutostartIndex)
        .map(el => el.info.Name);
    ...
    if(folderDebugMode) { // This is the existing folderDebugMode, not FOLDER_VIEW_DEBUG_MODE
        ...
        element.setAttribute('href', 'data:text/plain;charset=utf-8,' + encodeURIComponent(JSON.stringify({
            version: (await $.get('/plugins/folder.view0/server/version.php').promise()).trim(),
            folders,
            unraidOrder,
            originalOrder: JSON.parse(await $.get('/plugins/folder.view0/server/read_unraid_order.php?type=docker').promise()),
            newOnes,
            order,
            containersInfo
        })));
...
const createFolder = (folder, id, positionInMainOrder, liveOrderArray, containersInfo, foldersDone) => {
    ...
    const labelMatches = orderSnapshotAtFolderStart.filter(el => folderLabelOf(containersInfo[el]?.Labels) === folder.name && !combinedContainers.includes(el));
    labelMatches.forEach(match => combinedContainers.push(match));

    if (FOLDER_VIEW_DEBUG_MODE) {
        console.log(`[FV2_DEBUG] createFolder (id: ${id}): Containers matched by 'folder.view0' label ('${folder.name}'):`, labelMatches);
    ...
    switch (folder.settings.preview) {
        ...
        case 3:
            addPreview = (folderTrId, ctid, autostart) => {
                ...
                tmpId.attr("id", "folder-" + tmpId.attr("id"));
                if(folder.settings.context === 2 || folder.settings.context === 0) {
                    // Compose / 3rd-party containers have no a.exec inside .appname, only bare text.
    ...
    for (const container_name_in_folder of combinedContainers) {
        ...
        if (!$containerTR.length || !$containerTR.hasClass('sortable')) {
            ...
            // Unraid only wraps the name in an <a> for dockerman-managed containers; compose and
            // 3rd-party containers render it as bare text. Read the .appname span itself so both match.
                            ...
                            case 2: 
                                ...
                                try {
                                    charts.push(new Chart($(`.cpu-grapth-${ct.shortId} > canvas`, tooltipDomEl).get(0), { 
                                        type: 'line',
                                        data: { datasets: [ { label: 'CPU', data: CPU, borderColor: getComputedStyle(document.documentElement).getPropertyValue('--folder-view0-graph-cpu'), backgroundColor: getComputedStyle(document.documentElement).getPropertyValue('--folder-view0-graph-cpu'), tension: 0.4, pointRadius: 0, borderWidth: 1 } ] },
                                        options: options
                                    }));
                                    charts.push(new Chart($(`.mem-grapth-${ct.shortId} > canvas`, tooltipDomEl).get(0), { 
                                        type: 'line',
                                        data: { datasets: [ { label: 'MEM', data: MEM, borderColor: getComputedStyle(document.documentElement).getPropertyValue('--folder-view0-graph-mem'), backgroundColor: getComputedStyle(document.documentElement).getPropertyValue('--folder-view0-graph-mem'), tension: 0.4, pointRadius: 0, borderWidth: 1 } ] },
                                        options: options
                                    }));
                            ...
                            default:
                                ...
                                try {
                                    charts.push(new Chart($(`.comb-grapth-${ct.shortId} > canvas`, tooltipDomEl).get(0), { 
                                        ...
                                        data: {
                                            datasets: [
                                                { label: 'CPU', data: CPU, borderColor: getComputedStyle(document.documentElement).getPropertyValue('--folder-view0-graph-cpu'), backgroundColor: getComputedStyle(document.documentElement).getPropertyValue('--folder-view0-graph-cpu'), tension: 0.4, pointRadius: 0, borderWidth: 1 },
                                                { label: 'MEM', data: MEM, borderColor: getComputedStyle(document.documentElement).getPropertyValue('--folder-view0-graph-mem'), backgroundColor: getComputedStyle(document.documentElement).getPropertyValue('--folder-view0-graph-mem'), tension: 0.4, pointRadius: 0, borderWidth: 1 }
                                            ]
                                        },
                                        options: options
                                    }));
    ...
    if(folder.settings.update_column) {
        // Dropping the update column hands its width to the storage cell next to it.
...
const folderAutostart = (el) => {
    ...
    for (const container of containers) {
        ...
        if (containerAutostartCheckbox) {
            ...
            if (FOLDER_VIEW_DEBUG_MODE) console.log(`[FV2_DEBUG] folderAutostart: Container ${$(container).find('.appname').text().trim() || 'N/A'}: current autostart=${cstatus}. Folder target status=${status}`);
...
const rmFolder = (id) => {
    ...
    async (c) => {
        ...
        await $.get('/plugins/folder.view0/server/delete.php?type=docker&id=' + id).promise();
...
window.loadlist = () => {
    ...
    folderReq = [
        // Get the folders
        $.get('/plugins/folder.view0/server/read.php?type=docker').promise(),
        // Get the order as unraid sees it
        $.get('/plugins/folder.view0/server/read_order.php?type=docker').promise(),
        // Get the info on containers, needed for autostart, update and started
        $.get('/plugins/folder.view0/server/read_info.php?type=docker').promise(),
        // Get the order that is shown in the webui
        $.get('/plugins/folder.view0/server/read_unraid_order.php?type=docker').promise()
    ];
...
$.get('/plugins/folder.view0/server/cpu.php').promise().then((data) => {
    cpus = parseInt(data);
    ...
    dockerload.addEventListener('message', (e_sse) => { // Renamed e to e_sse to avoid conflict
        // Unraid subscribes to /sub/dockerload with NchanSubscriber, whose emitter passes the
        // payload straight through as a string. Older EventSource-style delivery wraps it in
        // an event with a .data property, so accept both shapes.
        const payload = typeof e_sse === 'string' ? e_sse : e_sse?.data;

        if (typeof payload !== 'string' || !payload.trim()) {
            if (FOLDER_VIEW_DEBUG_MODE) console.warn('[FV2_DEBUG] dockerload: Message had no usable string payload. Skipping. Received:', e_sse);
            return; // Skip processing if data is not a string or is empty
        }

        if (FOLDER_VIEW_DEBUG_MODE) console.log('[FV2_DEBUG] dockerload: Message received:', payload);
        let load = {};
