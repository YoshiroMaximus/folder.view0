...
const importDocker = () => {
    ...
    input.onchange = (e) => {
        ...
        reader.onload = async (readerEvent) => {
            ...
            if(content.name) {
                await $.post('/plugins/folder.view0/server/create.php', { type: 'docker', content: JSON.stringify(content) });
            } else {
                for (const [id, folder] of Object.entries(content)) {
                    await $.post('/plugins/folder.view0/server/update.php', { type: 'docker', content: JSON.stringify(folder), id: id });
                }
            }
            populateTable();
        }
...
const importVm = () => {
    ...
    input.onchange = (e) => {
        ...
        reader.onload = async (readerEvent) => {
            ...
            if(content.name) {
                await $.post('/plugins/folder.view0/server/create.php', { type: 'vm', content: JSON.stringify(content) });
            } else {
                for (const [id, folder] of Object.entries(content)) {
                    await $.post('/plugins/folder.view0/server/update.php', { type: 'vm', content: JSON.stringify(folder), id: id });
                }
            }
            populateTable();
        }
    ...
    if (id) {
        ...
        async (c) => {
            ...
            for (const cid of Object.keys(dockers)) {
                await $.get('/plugins/folder.view0/server/delete.php?type=docker&id=' + cid).promise();
            }
            populateTable();
        });
    }
    ...
    if (id) {
        ...
        async (c) => {
            ...
            for (const cid of Object.keys(vms)) {
                await $.get('/plugins/folder.view0/server/delete.php?type=vm&id=' + cid).promise();
            }
            populateTable();
        });
    }
...
const fileManager = async (type) => {
    location.href = location.pathname + '/Browse?dir=/boot/config/plugins/folder.view0';
};