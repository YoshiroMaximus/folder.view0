...
const createFolders = async () => {
    ...
    if(folderDebugMode) {
        ...
        element.setAttribute('href', 'data:text/plain;charset=utf-8,' + encodeURIComponent(JSON.stringify({
            ...
            folders,
            unraidOrder,
            originalOrder: JSON.parse(await $.get('/plugins/folder.view0/server/read_unraid_order.php?type=vm').promise()),
            newOnes,
            order,
            vmInfo
        })));
...
const dropDownButton = (id) => {
    ...
    if (state) {
        ...
        $(`tr.folder-id-${id} > td > .folder-storage`).append($(`.folder-${id}-element`));
...
const rmFolder = (id) => {
    ...
    async (c) => {
        ...
        await $.get('/plugins/folder.view0/server/delete.php?type=vm&id=' + id).promise();
        loadedFolder = false;
...
window.loadlist = (x) => {
    ...
    folderReq = [
        // Get the folders
        $.get('/plugins/folder.view0/server/read.php?type=vm').promise(),
        // Get the order as unraid sees it
        $.get('/plugins/folder.view0/server/read_order.php?type=vm').promise(),
        // Get the info on VMs, needed for autostart and started
        $.get('/plugins/folder.view0/server/read_info.php?type=vm').promise(),
        // Get the order that is shown in the webui
        $.get('/plugins/folder.view0/server/read_unraid_order.php?type=vm').promise()
    ];
    loadlist_original(x);
};
