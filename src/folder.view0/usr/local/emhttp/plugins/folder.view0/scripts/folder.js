...
(async () => {
    ...
    // get folders
    let folders = JSON.parse(await $.get(`/plugins/folder.view0/server/read.php?type=${type}`).promise());
    ...
    if (type === 'docker') {
        typeFilter = (e) => {
            return {
                ...
                'Label': folderLabelOf(e.info.Config.Labels)
            }
        };
    } else if (type === 'vm') {
        typeFilter = (e) => {
    ...
    choose = Object.values(JSON.parse(await $.get(`/plugins/folder.view0/server/read_info.php?type=${type}`).promise())).map(typeFilter);
...
const submitForm = async (e) => {
    ...
    if (folderId) {
        await $.post('/plugins/folder.view0/server/update.php', { type: type, content: JSON.stringify(folder), id: folderId });
    } else {
        await $.post('/plugins/folder.view0/server/create.php', { type: type, content: JSON.stringify(folder) });
    }
